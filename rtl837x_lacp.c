...
/* The timers step every four system ticks (50 Hz) whatever the pace of the
 * main loop; a long pass is caught up on, a few steps at a time. */
#define LACP_TICK_STEP	(SYS_TICK_HZ / 50)
#define LACP_CATCH_UP	8
__xdata uint8_t lacp_tick_last;
...
__xdata uint8_t  lacp_scratch_lag;
...
{
	if (uip_len < sizeof(struct lacpdu_in)) {
		uip_len = 0;
		return;
	}
	uip_len = 0;

	if (LACP_I->ethertype != HTONS(SLOW_PROTO_ETHERTYPE)
...
{
	uint8_t n = LACP_CATCH_UP;
	while (n-- && TICKS_DUE(lacp_tick_last, LACP_TICK_STEP)) {
		lacp_tick_last += LACP_TICK_STEP;
		lacp_step();
	}
}
...
void lacp_lag_set(uint8_t lag, uint16_t ports) __banked
{
	if (lag >= LACP_NUM_LAGS)
		return;
	if (ports && ports == lacp_lag_ports[lag]) {
		lacp_fdb_update();
		return;
	}
	...
	/* Release this LAG's trunk: LACP's own, or the static one a group put
	 * under LACP still has. Taking the group out (ports == 0) leaves a static
	 * trunk alone. */
	if (lacp_members_last[lag] || ports) {
	...
	/* Adopt the new member ports (with the engine already up). A port that
	 * belonged to another LACP LAG leaves that LAG's candidate set first. */
	for (uint8_t i = machine.min_port; i <= machine.max_port; i++) {
		if (!(ports & port_bit(i)))
			continue;
		lacp_scratch_lag = lacp_port_lag[i];
		if (lacp_scratch_lag != LACP_LAG_NONE && lacp_scratch_lag != lag) {
			lacp_lag_ports[lacp_scratch_lag] &= ~port_bit(i);
			if (!lacp_lag_ports[lacp_scratch_lag]) {
				lacp_agg_valid[lacp_scratch_lag] = 0;
				if (lacp_members_last[lacp_scratch_lag]) {
					lacp_members_last[lacp_scratch_lag] = 0;
					port_lag_members_set(lacp_scratch_lag, 0);
				}
			}
		}
		lacp_port_lag[i] = lag;
	...
	lacp_mux_update();	/* a LAG that lost a port to this one drops it from its trunk */
