...
try {
  ...
  if (baselineIndex !== -1) {
    ...
    for (const path of paths) {
      const bytes = await getDevice(path);
      const name =
        path === "/config"
          ? "config.txt"
          : path.startsWith("/vlan.json")
            ? `vlan-${path.split("=")[1]}.json`
            : "vlanlist";
      const before = await readFile(resolve(baseline, name));
      if (path === "/config")
        assert(
          bytes.equals(before),
          "Saved configuration differs after update.",
        );
      else
        assert.deepEqual(
          JSON.parse(bytes),
          JSON.parse(before),
          `VLAN state differs: ${path}`,
        );
    }
    console.log(
      "PASS: saved configuration and active VLANs 1, 4, 5 including PVID match the pre-update snapshot.",
    );
  }
} finally {
  await browser.close();
