...
void send_stp(void) __banked
{
	...
	stp_byte_html(root_bridge.prio);
	stp_byte_html(root_bridge.ext);
	slen += strtox(outbuf + slen, "\",\"rootMac\":\"");
	for (j = 0; j < 6; j++)
		stp_byte_html(root_bridge.mac[j]);
	slen += strtox(outbuf + slen, "\",\"myMac\":\"");
	for (j = 0; j < 6; j++)
		stp_byte_html(uip_ethaddr.addr[j]);
	slen += strtox(outbuf + slen, "\",\"cost\":\"");
	stp_byte_html(root_bridge_cost >> 24);
	stp_byte_html(root_bridge_cost >> 16);
	stp_byte_html(root_bridge_cost >> 8);
	stp_byte_html(root_bridge_cost);
	...
	for (i = 0; i < STP_ENTITIES; i++) {
		...
		stp_byte_html(dsg ? (stp_dpid[i] >> 8) : stp_pprio[i]);
		stp_byte_html(dsg ? stp_dpid[i] : (i + 1));
