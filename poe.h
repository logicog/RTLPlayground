 ...
 * driver's business and does not leak out here. Small scalar parameters pass as function
 * arguments; multi-field status is returned in xdata globals (never function locals) to keep
 * the 8051 overlay segment small.
 */
...
/* Enable/disable PoE on a single port (`poe port <n> <on|off>`). port_num is 0-based;
 * port_on: 1 = enable, 0 = disable. */
 ...
 */
extern __xdata uint8_t poe_port_num;	// poe_get_port() input: port 0-based
