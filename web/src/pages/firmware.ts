...
import { validateImage, type Info, type VlanList } from "../api";
import { uploadFirmware } from "../domain/firmware-upload";

type Phase = "idle" | "checking" | "upload" | "complete" | "unknown";
...
export class FirmwarePage extends Page {
  ...
  private async submitUpload(event: Event) {
    ...
    try {
      ...
      this.failure = (error as Error).message;
      if (this.phase === "checking" || import.meta.env.DEV) {
        this.phase = "idle";
    ...
    return html`
      <label class="check">
        <input
          ...
          required
        />
        I have saved settings and verified recovery for this device.
      </label>
      <label class="check">
        <input
          type="checkbox"
