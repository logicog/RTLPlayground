...
function createBW() {
  ...
  if (tbl.rows.length <= 2  && numPorts) {
     ...
     for (let i = 2; i < 2 + numPorts; i++) {
       ...
       td = tr.insertCell();
       td.innerHTML = '不限制';
       var button = '<button type="button" id="bwapply_' + i + '" style="margin: 0 0 0 24px" onclick="applyBandwidth(' + i + ');">应用</button>';
       td = tr.insertCell();
       td.innerHTML = button;
