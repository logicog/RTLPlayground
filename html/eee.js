    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (tbl.rows.length > 2) {
        for (let i = 2; i < 8; i++) {
          ...
          let tr = tbl.rows[n+1];
          if (!p.isSFP) {
            tr.cells[1].innerHTML = `${p.eee_2g5}`; tr.cells[2].innerHTML = `${p.eee_1g}`; tr.cells[3].innerHTML = `${p.eee_100m}`;
            tr.cells[4].innerHTML = `${p.eee_lp2g5}`; tr.cells[5].innerHTML = `${p.eee_lp1g}`; tr.cells[6].innerHTML = `${p.eee_lp100m}`;
