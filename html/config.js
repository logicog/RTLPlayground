...
const conf_cmds = [
  ...
  /^stp\s+trk\s+[1-4]\s+(on|off)$/,
  /^stp\s+trk\s+[1-4]\s+edge\s+(on|off|auto)$/,
  /^stp\s+trk\s+[1-4]\s+cost\s+\d{1,9}$/,
  /^stp\s+trk\s+[1-4]\s+prio\s+\d{1,3}$/,
  /^stp\s+trk\s+[1-4]\s+guard\s+(none|bpdu|root)$/,
  /^stp\s+trk\s+[1-4]\s+filter\s+(on|off)$/,
  /^stp\s+trk\s+[1-4]\s+p2p\s+(auto|on|off)$/,
  /^igmp\s+(on|off)$/,
  /^mtu\s+\d{1,2}\s+\d+$/,
...
const conf_overwrite = [
  ...
  /^stp\s+trk\s+[1-4]\s+(edge|cost|prio|guard|filter|p2p)\b/,
  /^igmp\b/,
  /^mtu\s+\d{1,2}\b/,
