...
of Layer-3 MC packets on the LAN, which otherwise would be flooded across the
...
There is presently no support for IPv6/MLD-based MC.
...
them. While there is support in the HW to handle IGMP/MLD packets (v3 has only limited
support) entirely in hardware and even send out reports, it is currently not understood
...
switch ports, including flooding your home network.
