...
| Flash | Winbond W25Q80DV, 8 MBit / 1 MiB | PCB |
...
- The board has no reset button (confirmed on the case), so `reset_pin = GPIO_NA` is correct.
...
Chip port 8 is SerDes 1 with nothing attached, and `rtl8373_init()` disables it. It exists in the
port list only because RTLPlayground assumes the CPU port is `max_port + 1`. Every MDIO read at its
PHY address returns 0xffff, so `port 9 show` decodes all-ones as a link partner advertising every
speed at both duplexes, and the port state "Off" comes from the same floating read. The link status
register `0x63e8` has the bit for that port clear, so the port really is down. The `<new link:
2000...>` line at boot is the stale speed nibble in `0x63f4` settling during init. All cosmetic.

### Installation

Unmanaged: SOIC-8 clip on the W25Q80DV only. Dump and keep the stock image first.
Build with `make MACHINE=LINKSYS_LN2308`.
Updates after the first install go through the web interface (`/upload`); verified on this board,
the 1 MiB flash is exactly the minimum the updater requires.
