...
- The `9(10G)` lamp on the front panel is not driven by the LED controller.
  It stays dark for every LED configuration, so it is either wired to the SFP
  module or not connected on this board. The `SFP` lamp works.
